...
def greedy_BFS(start_station_name: str, end_station_name: str, map: dict[str, Station]) -> List[
    str]:
    """
    ...
    while not frontier.empty():
        ...
        for next in current.links:
            if next not in came_from:
                priority = Euclidean_distance(end_station.position[0], next.position[0],
                                              end_station.position[1], next.position[1])
...
if __name__ == '__main__':
    ...
    path = greedy_BFS(start_station_name, end_station_name, stations)
    end_time = timeit.default_timer()

    # 计算执行时间
    print(f"greedy BFS 执行时间：{(end_time - start_time) * 1000:.3f} ms")

    # visualization the path
    # Open the visualization_underground/my_path_in_London_railway.html to view the path, and your path is marked in red
    plot_path(path, 'visualization_underground/greedy BFS_in_London_railway.html', stations,
              underground_lines)
