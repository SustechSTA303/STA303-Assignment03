...
def build_data():
    ...
    for id1, id2, lineNumber in r:
        ...
        if lineNumber not in underground_lines:
            underground_lines[lineNumber] = {
                "lat": [stations[id1].position[0], stations[id2].position[0], None],
                "lon": [stations[id1].position[1], stations[id2].position[1], None],
                "stations": {stations[id1].name, stations[id2].name},
            }
        else:
            underground_lines[lineNumber]["lat"].extend(
                [stations[id1].position[0], stations[id2].position[0], None]
            )
            underground_lines[lineNumber]["lon"].extend(
                [stations[id1].position[1], stations[id2].position[1], None]
            )
